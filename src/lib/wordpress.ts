...
// How long a WordPress response is reused before it is fetched again
// (in seconds). 60 = new posts and edits show up within about a minute,
// with no redeploy needed.
const REVALIDATE_SECONDS = 60;

// Small in-memory cache so a busy page does not hit WordPress on every view.
const cache = new Map<string, { at: number; data: unknown }>();

async function wpFetch<T>(path: string): Promise<{ ok: boolean; status: number; data: T | null }> {
...
function extractHeadingsAndTagContent(html: string): {
  ...
  const tagged = html.replace(
    ...
    (match, attrs: string, inner: string) => {
      ...
      const innerWithColor = numberMatch?.[1]
...
function toAuthor(raw?: RawWPAuthor): WPAuthor {
  ...
  const author: WPAuthor = {
    ...
    avatar: largest ? (avatarUrls[largest] ?? null) : null,
...
export async function getPosts(page = 1, perPage = 10): Promise<WPPostSummary[]> {
  try {
    const res = await wpFetch<RawWPPost[]>(
      `/wp-json/wp/v2/posts?${EMBED_PARAM}&per_page=${perPage}&page=${page}&status=publish`,
    );
...
export async function getPostBySlug(slug: string): Promise<WPPost | null> {
  try {
    const res = await wpFetch<RawWPPost[]>(
      `/wp-json/wp/v2/posts?${EMBED_PARAM}&slug=${encodeURIComponent(slug)}&status=publish`,
    );
...
/** Every published post's slug and last-modified date, for sitemap.xml.
 *  Pages through the API 100 at a time (WordPress's maximum per request). */
export async function getSitemapPosts(): Promise<{ slug: string; modified: string }[]> {
  const all: { slug: string; modified: string }[] = [];
  try {
    for (let page = 1; page <= 20; page++) {
      const res = await wpFetch<{ slug: string; modified: string }[]>(
        `/wp-json/wp/v2/posts?per_page=100&page=${page}&status=publish&_fields=slug,modified`,
      );
    ...
    console.error("[wordpress] error fetching sitemap posts", err);
  }
  return all;
}
